...
const MoviesCom:FC<DataProps> = ({ data }) => {
     ...
     const searchMovies = async () => {
          if (searchTerm !== '') {
               await axios.get(`https://moviesapi.ir/api/v1/movies/?q=${searchTerm}`)
                    .then((res) => {
     ...
     const handleSelect = (e:ChangeEvent<HTMLSelectElement>) => {
          if (e.target.value === "") {
               ...
               setSelectedOption(e.target.value);
               setInitialItem(item.filter((product) => product.country.indexOf(e.target.value) >= 0))
          }
     };
     ...
     const sortMovies = (e:ChangeEvent<HTMLInputElement>) => {
          ...
          if (sort === "new") {
               setInitialItem(initialItem.sort((a:MovieData, b:MovieData) => (a.id < b.id ? 1 : -1)))
          }
          if (sort === "old") {
     ...
     return (
          <div>
               <div className={styles.parentMovies}>
                    <FilterSearch handleSearch={handleSearch} searchMovies={searchMovies} searchTerm={searchTerm} />
                    <FilterSelect selectedOption={selectedOption} handleSelect={handleSelect} sortMovies={sortMovies} />
               </div>
               ...
               <ReactPaginate 
                    ...
                    nextClassName={`${currentPage === Math.ceil(initialItem.length / moviesPerPage) - 1 ? styles.invisible : ''} ${styles.paginationPrevNext}`}
                     previousClassName={`${currentPage === 0 ? styles.invisible : ''} ${styles.paginationPrevNext}`}
               />
